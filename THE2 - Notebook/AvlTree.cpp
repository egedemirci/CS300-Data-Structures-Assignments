...
void AvlTree<Comparable>::remove(const Comparable & x, AvlNode<Comparable> * & t ) const
{
    ...
    if (x < t->element) /*classic bst remove.*/
        remove(x, t->left);
    else if (t->element < x)
        remove(x, t->right);
	...
	if(height( t->left ) - height( t->right ) > 1){ /*If there is a unbalance. */
		if (height(t->left->left) < height(t->left->right)){ /*if tree's left subtree is right heavy. */
			doubleWithLeftChild(t);
		}
		else{ /*if tree's left subtree is left heavy. */
			rotateWithLeftChild(t);
		}
	}
	else{
		if (height(t->right) - height(t->left ) > 1){
			if (height(t->right->right) < height(t->right->left)){ /*if tree's right subtree is left heavy. */
				doubleWithRightChild(t);
			}
			else{
				rotateWithRightChild(t); /*if tree's right subtree is right heavy. */
			}
		}
...
template <class Comparable>
void AvlTree<Comparable>::updateTree(const Comparable& oldOne, const Comparable& newOne){
	AvlNode<Comparable> * elementToBeUpdated = find(oldOne,root);
